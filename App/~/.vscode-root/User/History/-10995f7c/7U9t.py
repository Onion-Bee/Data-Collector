...
style = ttk.Style()
style.theme_use("clam")  # Options: 'clam', 'alt', 'default', etc.
...
def submit_info():
    ...
    # Destroy the kid-info window then launch the initial screening.
    root.destroy()
    show_initial()  # Launch next phase

# Create the kid info window using a grid layout.
root = tk.Tk()
...
# Define initial screening questions.
initial_questions = [
...
# Define follow-up screening questions.
followup_questions = [
...
def show_initial():
    ...
    frame = ttk.Frame(canvas, padding="10 10 10 10", style="TFrame")
    canvas.create_window((0, 0), window=frame, anchor="nw")

    # Header label.
    ttk.Label(frame, text="Please answer the following questions:",
              font=("Arial", 14, "bold"), background="#f7f7f7").grid(column=0, row=0, columnspan=2, pady=(0, 15))
    initial_var_list = []
