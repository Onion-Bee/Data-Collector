...
# Improved GUI for Kid Info - Revised layout
root = tk.Tk()
root.title("Child Information Entry")
root.geometry("400x350")  # Reduced window height
root.configure(bg=BG_COLOR)
root.resizable(False, False)
...
main_frame = ttk.Frame(root, padding=20)
main_frame.pack(fill="both", expand=True)
main_frame.grid_rowconfigure(4, weight=1)  # Allow vertical expansion

# Title
ttk.Label(main_frame, text="Child Information", 
         font=(FONT_NAME, 16, "bold"), foreground=PRIMARY_COLOR).grid(row=0, column=0, pady=(0, 15), columnspan=2)

# Form fields using grid
fields = [
    ("Full Name:", 1, name_entry := ttk.Entry(main_frame, width=22)),
    ("Age:", 2, age_entry := ttk.Entry(main_frame, width=22)),
    ("Gender:", 3, gender_entry := ttk.Entry(main_frame, width=22))
]

for label_text, row, entry_widget in fields:
    ttk.Label(main_frame, text=label_text).grid(row=row, column=0, padx=5, pady=7, sticky="e")
    entry_widget.grid(row=row, column=1, padx=5, pady=7, sticky="w")

# Submit button
submit_button = ttk.Button(main_frame, text="Submit Information", command=submit_info)
submit_button.grid(row=4, column=0, columnspan=2, pady=(20, 10))

# Status label
info_status_label = ttk.Label(main_frame, text="", wraplength=300, justify="center")
info_status_label.grid(row=5, column=0, columnspan=2, pady=(0, 10))
