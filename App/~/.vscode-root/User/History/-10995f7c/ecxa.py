...
# Use a ttk style (choose a modern theme)
style = ttk.Style()
style.theme_use("clam")  # alternatives: 'clam', 'alt', 'default', 'vista' (Windows)
...
# Create the kid info window with a grid layout
root = tk.Tk()
root.title("Kid Information")
...
# Define 20 initial screening questions
initial_questions = [
...
def show_initial():
    ...
    # Header label
    ttk.Label(frame, text="Please answer the following questions:", font=("Arial", 14, "bold"),
              background="#f7f7f7").grid(column=0, row=0, columnspan=2, pady=(0, 15))
    initial_var_list = []
    for idx, question in enumerate(initial_questions):
        ttk.Label(frame, text=question, background="#f7f7f7", wraplength=500, font=("Arial", 10)).grid(column=0, row=idx+1, sticky="w", padx=5, pady=3)
        ...
        ttk.Radiobutton(options_frame, text="Yes", variable=var, value="Yes").pack(side="left", padx=2)
        ttk.Radiobutton(options_frame, text="No", variable=var, value="No").pack(side="left", padx=2)

    ttk.Button(frame, text="Submit Answers", command=submit_initial).grid(column=0, row=len(initial_questions)+2, columnspan=2, pady=20)
...
def show_followup():
    ...
    main_frame.grid(column=0, row=0, sticky=(tk.N, tk.W, tk.E, tk.S))
    ttk.Label(main_frame, text="Follow-Up Questions:", font=("Arial", 14, "bold")).grid(column=0, row=0, columnspan=2, pady=(0,15))
    followup_var_list = []
    for idx, question in enumerate(followup_questions):
        ttk.Label(main_frame, text=question, wraplength=500, font=("Arial", 10)).grid(column=0, row=idx+1, sticky="w", padx=5, pady=5)
        var = tk.StringVar(value="Yes")
    ...
    ttk.Button(main_frame, text="Submit Follow-Up Answers", command=submit_followup).grid(column=0, row=len(followup_questions)+1, columnspan=2, pady=20)
