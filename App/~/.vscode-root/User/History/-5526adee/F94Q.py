    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            success, rot_vec, trans_vec = cv.solvePnP(
                head_pose_points_3D.astype(np.float64),
                np.delete(head_pose_points_3D, 2, axis=1).astype(np.float64),
                cam_matrix, dist_matrix
            )
            rotation_matrix, _ = cv.Rodrigues(rot_vec)
            angles, _, _, _, _, _ = cv.RQDecomp3x3(rotation_matrix)
            angle_x, angle_y, z = angles * 360
            ...
            if SHOW_ON_SCREEN_DATA:
                cv.putText(frame, f"Face Looking at {face_looks}", (30, 30), cv.FONT_HERSHEY_DUPLEX, 0.8, (0,255,0), 2)
            ...
            # Highlight irises and corners
            cv.circle(frame, center_left, int(l_radius), (255,0,255), 2)
            cv.circle(frame, center_right, int(r_radius), (255,0,255), 2)
            cv.circle(frame, mesh_points[LEFT_EYE_INNER_CORNER][0], 3, (255,255,255), -1)
            cv.circle(frame, mesh_points[LEFT_EYE_OUTER_CORNER][0], 3, (0,255,255), -1)
            cv.circle(frame, mesh_points[RIGHT_EYE_INNER_CORNER][0], 3, (255,255,255), -1)
            cv.circle(frame, mesh_points[RIGHT_EYE_OUTER_CORNER][0], 3, (0,255,255), -1)
            ...
            if PRINT_DATA:
                ...
                if ENABLE_HEAD_POSE:
                    ...
                    if initial_pitch is None or key == ord('c'):
