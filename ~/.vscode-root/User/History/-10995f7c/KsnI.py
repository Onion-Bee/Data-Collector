...
root = tk.Tk()
root.withdraw()  # Hide the default blank window.
...
def show_initial():
    ...
    canvas = tk.Canvas(window_initial, borderwidth=0, width=530, height=550)
    ...
    ttk.Label(frame, text="Please answer the following questions:",
              font=("Arial", 14, "bold")).grid(column=0, row=0, columnspan=2, pady=(0, 15))
    initial_var_list = []
    for idx, question in enumerate(initial_questions):
        ttk.Label(frame, text=question, wraplength=500, font=("Arial", 10))\
