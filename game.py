...
class Bubble:
    ...
    def draw(self):
        ...
        pygame.draw.circle(shadow, (0, 0, 0, int(alpha*0.3)),
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type in (pygame.MOUSEBUTTONDOWN, pygame.FINGERDOWN):
            pos = (pygame.mouse.get_pos() if event.type == pygame.MOUSEBUTTONDOWN
    ...
    box = pygame.Surface((score_surf.get_width()+20,
                          score_surf.get_height()+10), pygame.SRCALPHA)
...
box_e.blit(score_msg, ((bw-score_msg.get_width())//2,
...
# Determine target folder for CSV
try:
...
# Save CSV inside the child folder (or logs/)
ts = datetime.now().strftime("%Y%m%d_%H%M%S")
filename = os.path.join(target_dir, f"reaction_times_{ts}.csv")
